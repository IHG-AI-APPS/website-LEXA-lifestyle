...
export default function AIChatWidget() {
  ...
  return (
    <>
      {/* AURA Chat Button — above quick actions on mobile, above WhatsApp on desktop */}
      <motion.button
        data-testid="ai-chat-button"
        onClick={() => setIsOpen(true)}
        className={`fixed bottom-[148px] lg:bottom-[104px] right-4 lg:right-6 z-40 w-14 h-14 rounded-full overflow-hidden shadow-2xl ring-2 ring-[#C9A962]/40 hover:ring-[#C9A962]/70 transition-all chat-pulse ${isOpen ? 'hidden' : ''}`}
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-4 right-4 z-50 w-[400px] max-w-[calc(100vw-32px)] h-[620px] max-h-[calc(100vh-100px)] sm:max-h-[calc(100vh-100px)] bg-white dark:bg-[#0A0A0A] rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-gray-200 dark:border-zinc-800"
          >
            {/* Header */}
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50 dark:bg-[#171717]">
                  ...
                  <div className="space-y-2">
                    {[
                      ...
                      <button
                        ...
                        className="block w-full text-left text-sm px-3 py-2.5 bg-white dark:bg-gray-700 rounded-lg border border-gray-200 dark:border-zinc-700 hover:border-[#C9A962] hover:bg-amber-50/50 dark:hover:bg-amber-900/20 transition-colors dark:text-gray-200"
                      >
                        {suggestion}
                      </button>
                    ))}
                  ...
                  <div className={`flex items-start gap-2 max-w-[85%] ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                    ...
                    <div className={`px-4 py-2.5 rounded-2xl ${
                      msg.role === 'user' 
                        ...
                        : 'bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-tl-sm shadow-sm border border-gray-100 dark:border-zinc-700'
                    }`}>
              ...
              {isLoading && (
                ...
                >
                  <div className="flex items-center gap-2 px-4 py-2.5 bg-white dark:bg-gray-700 rounded-2xl rounded-tl-sm shadow-sm border border-gray-100 dark:border-zinc-700">
                    <Loader2 className="w-4 h-4 animate-spin text-[#C9A962]" />
                    <span className="text-sm text-gray-500 dark:text-zinc-500">AURA is thinking...</span>
                  </div>
                </motion.div>
              )}
            ...
            {leadScore > 0 && (
              <div className="px-4 py-2 bg-gradient-to-r from-amber-50 to-yellow-50 dark:from-amber-900/20 dark:to-yellow-900/20 border-t border-gray-100 dark:border-zinc-800">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-gray-600 dark:text-zinc-500">Consultation Progress</span>
            ...
            <div className="p-4 border-t border-gray-200 dark:border-zinc-800 bg-white dark:bg-[#0A0A0A]">
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="flex-1 px-4 py-2.5 border border-gray-200 dark:border-zinc-700 rounded-full focus:outline-none focus:border-[#C9A962] focus:ring-2 focus:ring-[#C9A962]/20 text-sm bg-white dark:bg-[#171717] dark:text-white"
                  disabled={isLoading}
                />
                <button
