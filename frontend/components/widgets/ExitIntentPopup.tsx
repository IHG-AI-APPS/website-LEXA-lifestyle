    ...
    <AnimatePresence>
      {isVisible && (
        <>
          ...
          >
            <div className="bg-white rounded-xl shadow-2xl overflow-hidden w-full max-w-sm sm:max-w-md mx-auto my-auto relative">
              {/* Close button */}
              <button
                onClick={close}
                data-testid="exit-popup-close"
                className="absolute top-3 right-3 p-1.5 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-full z-10 transition-colors"
              >
                <X className="w-4 h-4" />
              </button>
              ...
              {!isSubmitted ? (
                ...
                <div className="p-4 sm:p-6 text-center">
                  ...
                  <h3 className="text-lg sm:text-xl font-bold text-gray-800 mb-1">You're All Set!</h3>
                  <p className="text-gray-600 text-xs sm:text-sm">{variant.success_message}</p>
                </div>
              )}
            </div>
          </motion.div>
