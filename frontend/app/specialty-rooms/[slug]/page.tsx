...
export default function SpecialtyRoomDetailPage() {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#050505] pt-20" data-testid="specialty-room-detail-page">
      {/* Hero — Split Layout */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 min-h-[480px]">
          <div className="flex flex-col justify-center px-8 lg:px-16 py-16 relative z-10">
            <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }}>
              <span className="hero-animate-badge inline-block px-2.5 py-1 sm:px-3 rounded-full bg-[#C9A962]/15 border border-[#C9A962]/30 text-[#C9A962] text-xs uppercase tracking-widest mb-5" data-testid="room-category">
                {room.category}
              </span>
              <h1 className="hero-animate-title text-3xl sm:text-4xl lg:text-5xl font-bold uppercase mb-5 tracking-tight leading-tight" data-testid="room-title">
                {room.name}
              </h1>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-12">
              <div className="lg:col-span-3">
                ...
                {room.long_description && (
                  <p className="text-base text-gray-600 dark:text-zinc-400 leading-relaxed mb-8">{room.long_description}</p>
                )}
                {features.length > 0 && (
      ...
      {brands.length > 0 && (
        <section className="py-16 lg:py-20 bg-white dark:bg-[#050505]" data-testid="brands-section">
          <div className="container mx-auto px-5 sm:px-8 lg:px-16">
                ...
                {brands.map((brand: string, i: number) => (
                  <motion.div key={i} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.2, delay: i * 0.04 }} className="group">
                    <Link href={`/brands/${brand.toLowerCase().replace(/\s+/g, '-')}`}
                      className="flex flex-col items-center justify-center p-4 h-20 rounded-xl bg-gray-50 dark:bg-[#0A0A0A] border border-gray-200 dark:border-zinc-800 hover:border-[#C9A962]/60 hover:shadow-md transition-all">
      ...
      {relatedSolutions.length > 0 && (
        <section className="py-16 lg:py-20 bg-gray-50 dark:bg-[#0A0A0A]" data-testid="related-solutions-section">
          <div className="container mx-auto px-5 sm:px-8 lg:px-16">
            <div className="max-w-6xl mx-auto">
              <div className="text-center mb-12">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {relatedSolutions.map((sol: string, i: number) => (
                  <Link key={i} href={`/solutions/${sol}`}
                    className="group block p-6 rounded-xl bg-white dark:bg-[#050505] border border-gray-200 dark:border-zinc-800 hover:border-[#C9A962]/60 hover:shadow-lg transition-all">
