...
function renderContent(text: string) {
  return text.split('\n').map((line, i) => {
    ...
    if (line.startsWith('- **')) {
      ...
      return <li key={i} className="text-gray-600 dark:text-zinc-500"><strong>{parts[0].replace(/\*\*/g, '')}:</strong> {parts[1]}</li>
    }
...
export default function PrivacyPolicyContent() {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#050505] pt-20" data-testid="privacy-policy-page">
      {/* Hero */}
      <section className="relative overflow-hidden bg-[#0A0A0A] dark:bg-[#050505] text-white py-16 lg:py-24">
        <div className="container mx-auto px-5 sm:px-8 lg:px-16 relative z-10">
          <div className="max-w-3xl mx-auto text-center">
            <span className="hero-animate-badge inline-block px-2.5 py-1 sm:px-3 rounded-full bg-[#C9A962]/15 border border-[#C9A962]/30 text-[#C9A962] text-xs uppercase tracking-widest mb-5">Legal</span>
            <h1 className="hero-animate-title text-3xl sm:text-4xl lg:text-5xl font-bold uppercase mb-5 tracking-tight" data-testid="privacy-policy-title">{title}</h1>
